...
function install_apt_dependencies()
{
    ...
    exec_and_search_errors "$SUDO_PREFIX apt-get $APT_QUIET install --fix-missing -y $(cat "$DIR/requirements_apt.txt")"
...
function install_python_dependencies()
{
    ...
    if [ "${OS_DISTRO,,}" = "ubuntu" ] ; then
        ...
        exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET --upgrade numpy>=1.13.0,<=1.13.3"
        ...
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET Cython"
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET graphviz"
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET scikit-image"
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET --upgrade numpy==1.13.1"
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET pygraphviz Enum34>=1.1.6 networkx>=2.1,<=2.1"
        # Install packages for python 2.x, required for NCSDK python API
        exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET Enum34>=1.1.6"
        exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET --upgrade numpy==1.13.1"
    fi
}
...
function install_tensorflow()
{
    ...
    if [[ $tf -ne 0 && $tf_gpu -ne 0 ]]; then
        ...
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET tensorflow==$SUPPORTED_TENSORFLOW_VERSION"
    else
